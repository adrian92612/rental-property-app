...
describe("user credential registration", () => {
  ...
  it("should validate client side input", () => {
    ...
    cy.realPress("Tab");
    cy.get("p").contains(emailRequired).should("be.visible");
    cy.get('input[name="email"]').type("invalid email");
    cy.realPress("Tab");
    cy.get("p").contains(emailInvalid).should("be.visible");
    ...
    cy.realPress("Tab");
    cy.get("p").contains(firstNameRequired).should("be.visible");
    ...
    cy.realPress("Tab");
    cy.get("p").contains(lastNameRequired).should("be.visible");
    ...
    cy.realPress("Tab");
    cy.get("p").contains(invalidPassword).should("be.visible");
    ...
    cy.realPress("Tab");
    cy.get("p").contains(notMatchPassword).should("be.visible");
    cy.get('input[name="confirmPassword"]').type("Password123");
    cy.realPress("Tab");
    cy.get("p").contains(notMatchPassword).should("be.visible").as("p-element");
